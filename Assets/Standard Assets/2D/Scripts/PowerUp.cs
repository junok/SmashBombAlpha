...
public class PowerUp : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D trigger)
    {
        if (trigger.gameObject.tag == "PowerUp")
        {
            ...
            if (mode == 0)
            {
                this.GetComponentInChildren<Weapon>().bulletSpeed = 50;
                this.GetComponentInChildren<Weapon>().bulletLifeTime = 1.5f;
                this.GetComponentInChildren<PlayerKnockBack>().knockBack = 30;
            }
            else if (mode == 1)
                this.GetComponentInChildren<Weapon>().bulletSpeed = sniperPower;
            else if (mode == 2)
                this.GetComponentInChildren<Weapon>().bulletLifeTime = longerBombPower;
            else if (mode == 3)
